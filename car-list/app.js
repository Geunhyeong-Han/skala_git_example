...
confirmOkButton.addEventListener('click', confirmDelete)
confirmCancelButton.addEventListener('click', hideConfirmModal)

// 검색어를 입력할 때마다 목록을 다시 그린다.
searchInput.addEventListener(/* TODO: [검색어 입력시 목록 재 출력] 수강생 구현 #3 */"input", renderCars);

// 판매 상태를 바꿀 때마다 목록을 다시 그린다.
statusFilter.addEventListener(/* TODO: [판매 상태 변경시 목록 재 출력] 수강생 구현 #4 */"change", renderCars);
...
function getCarFromForm() {
  ...
  if (year === "" || year < 1900 || year > maxYear) {
    alert(`연식은 ${1900}년부터 ${maxYear}년(현재 년도) 사이로 입력해주세요.`);
    yearInput.focus();
    return null;
  }
...
function deleteCar(id) {
  ...
  showConfirmModal('선택한 차량을 삭제할까요?', id);
}